...
const EntityRegions = (props) => {
    ...
    React.useEffect(() => {
        regions
            .filter((region) => getHiddenRegionState(region.id) === undefined)
            .forEach((region) => setRegionVisibility(getUniqueRegionID(region.id), region.initialVisibility));
    }, [inputRegions, isHiddenRegion, getHiddenRegionState, setRegionVisibility, getUniqueRegionID, regions]);

    React.useEffect(() => {
        const defaultVisibility = (region) =>
            regionMaximized === region.id ||
            visibleRegionsParam.includes(region.id) ||
            (!visibleRegionsParam.length &&
                !isHiddenRegion(region.id) &&
                (region.customVisibility ? region.customVisibility() : true));
    ...
    React.useEffect(
        ...
        [searchParams[ENTITY_REGION_PARAMS.MAXIMIZE]] // eslint-disable-line react-hooks/exhaustive-deps
    );

    const columns = regions.reduce(
        (acc, region) => ({
            ...acc,
            [region.column]: [...(acc[region.column] || []), region],
        }),
        {}
    );
    ...
    const getRegionPanelQueryParams = React.useCallback(
        (regionID) =>
            Object.keys(searchParams)
                ...
                {Object.keys(columns)
                    ...
                    .map((column) => (
                        <Grid
                            key={column}
                            item
                            xs={gridDimensions.xs}
                            sm={gridDimensions.sm}
                            md={gridDimensions.md}
                            lg={gridDimensions.lg}
                        >
                            {columns[column]
                                ...
                                .map((region) => (
                                    <RegionPanel
                                        style={{
                                            display: regionMaximized && region.id !== regionMaximized ? 'none' : '',
                                        }}
                                        key={region.id}
